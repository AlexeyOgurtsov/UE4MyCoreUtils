...
#define M_DECLARE_CUSTOM_SCOPED_LOG_HELPER_CLASS_IF(ClassNamePrefix, LogCategory, LogLevel)\
	...
	:		bShouldLog(bInShouldLog), PrefixString(InPrefixString)\
		{\
			M_LOG_CUSTOM_TO_IF(bShouldLog, LogCategory, LogLevel, TEXT("%s : Block entered"), *PrefixString);\
		}\
		~M_CUSTOM_SCOPED_LOG_HELPER_CLASS_NAME(ClassNamePrefix)()\
		{\
			M_LOG_CUSTOM_TO_IF(bShouldLog, LogCategory, LogLevel, TEXT("%s : Exiting block"), *PrefixString);\
		}\
	private:\
		bool bShouldLog;\
...
#define M_LOGFUNC_NAMED_STRING_IF_TO(InName, ShouldLog, LogCategory, InString)\
	...
	M_CUSTOM_SCOPED_LOG_HELPER_CLASS_NAME(InName) Autogenerated_##InName##_ScopedLogHelper {ShouldLog, InString};
