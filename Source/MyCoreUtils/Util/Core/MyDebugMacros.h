...
#define M_DEBUG_LOGFUNC_PREFIX (FString(__FILE__) + FString(TEXT(":")) + FString(__FUNCTION__) + FString(TEXT(":")) + FString::FromInt(__LINE__) + FString(TEXT(": "))) 
// ~String debug macros End

// ~Logging macros Begin
#define M_LOG_CUSTOM_TO(LogCategory, LogLevel, FormatString, ...)\
{\
	UE_LOG(LogCategory, LogLevel, TEXT("%s%s"),  *M_DEBUG_LOGFUNC_PREFIX, *FString::Printf(FormatString, ##__VA_ARGS__));\
}
...
#define M_LOGFUNC_NAMED_STRING_IF_TO(InName, ShouldLog, LogCategory, InString)\
	...
	M_CUSTOM_SCOPED_LOG_HELPER_CLASS_NAME(InName) Autogenerated_##InName##_ScopedLogHelper {ShouldLog, M_DEBUG_LOGFUNC_PREFIX + InString};
